require_relative "../lib/shoulda/context/version"
require "erb"
...
module Shoulda
  module Context
    module DocumentationTasks
      ...
      def self.create
        ...
        namespace :docs do
          ...
          file(
            DocumentationPublisher.docs_dir => DocumentationPublisher.gh_pages_dir
          )
    ...
    class DocumentationPublisher
      CURRENT_VERSION = Shoulda::Context::VERSION
      ...
      def create_reference_to_gh_pages_branch
        system(*%W(
          git
          clone
          git@github.com:#{GITHUB_USERNAME}/shoulda-context.git
          #{GH_PAGES_DIR}
          --branch
          gh-pages
        ))
      ...
      def add_version_to_index_page_for(ref, version)
        within_docs_dir do
          ...
          content.sub!(
            %r{<h1>shoulda-context.+</h1>},
            "<h1>shoulda-context (#{version})</h1>"
          )
